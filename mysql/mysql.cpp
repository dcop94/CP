...
int main()
{
    ...
    while (res->next())
    {
        cout << "id = " << res->getInt("id") << ", name = " << res->getString("name") << ", quantity = " << res->getInt("quantity") << endl;
    }
    delete res;
    delete stmt;

    pstmt = con->prepareStatement("UPDATE inventory SET name = ? WHERE id = ?; ");
    pstmt->setString(1, "guaba");
    pstmt->setInt(2, 1);
    pstmt->executeUpdate();
    printf("Row update \n");
